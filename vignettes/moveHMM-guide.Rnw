...
% for graphs
\usepackage{tikz}
\usetikzlibrary{arrows,automata,decorations}
\usetikzlibrary{decorations.pathreplacing}
% decorations.markings for arrow heads in the middle of edges
\usetikzlibrary{decorations.markings}
% angles to draw angle arcs, quotes to label them
...
For full details of the HMM approach in the context of animal movement modelling, the user should refer to the relevant primary publications (see, e.g., \citealp{patterson2009}, \citealp{langrock2012}, \citealp{zucchini2009}). Here, we briefly highlight the essential features of the HMM approach. 
...
% for arrow head in the middle of edges
...
\begin{figure}[ht]
  \centering
  \resizebox{0.44\textwidth}{!}{ % scale the whole tikz picture
    \begin{tikzpicture}[]
      
      \node[circle,scale=0.3,fill,label={270:$(x_{t-1},y_{t-1})$}] at (0,0) (a) {};
      \node[circle,scale=0.3,fill,label={90:$(x_{t},y_{t})$}] at (2,3) (b) {};
      \node[circle,scale=0.3,fill,label={180:$(x_{t+1},y_{t+1})$}] at (4,1) (c) {};
      \node[circle,scale=0.3,fill,label={90:$(x_{t+2},y_{t+2})$}] at (8,4) (d) {};
      \node[] at (3,4.5) (e) {};
      \node[] at (5,0) (f) {};				
      
      \draw[->-,line width=1pt] (a.center) -- (b.center);
      \draw[->-,line width=1pt] (b.center) -- (c.center);
      \draw[->-,line width=1pt] (c.center) -- (d.center);
      
      \draw[decorate,decoration={brace,amplitude=10pt},xshift=-4pt,yshift=4pt,red] (0,0) -- node [midway,xshift=-15pt,yshift=15pt] {$l_{t-1}$} (2,3);		
      \draw[decorate,decoration={brace,amplitude=10pt},xshift=4pt,yshift=4pt,red] (2,3) -- node [midway,xshift=15pt,yshift=15pt] {$l_{t}$} (4,1);		
      \draw[decorate,decoration={brace,amplitude=10pt,mirror},xshift=4pt,yshift=-4pt,red] (4,1) -- node [midway,xshift=15pt,yshift=-15pt] {$l_{t+1}$} (8,4);		
      
    \end{tikzpicture}
  }
  \resizebox{0.54\textwidth}{!}{ % scale the whole tikz picture
    \begin{tikzpicture}[]
      
      \node[circle,scale=0.3,fill,label={180:$(x_{t-1},y_{t-1})$}] at (0,0) (a) {};
      \node[circle,scale=0.3,fill,label={120:$(x_{t},y_{t})$}] at (2,3) (b) {};
      \node[circle,scale=0.3,fill,label={220:$(x_{t+1},y_{t+1})$}] at (4,1) (c) {};
      \node[circle,scale=0.3,fill,label={0:$(x_{t+2},y_{t+2})$}] at (8,4) (d) {};
      \node[] at (3,4.5) (e) {};
      \node[] at (5,0) (f) {};					
      ...
      \draw[dashed,->-,line width=1pt] (b.center) -- (e.center);
      \draw[dashed,->-,line width=1pt] (c.center) -- (f.center);
      
      \path (c) -- (b) -- (e) pic["$\phi_t$",draw,-latex,<->,angle radius=0.5cm,angle eccentricity=1.5,red]
      {angle=c--b--e};
      \path (f) -- (c) -- (d) pic["$\phi_{t+1}$",draw,-latex,<->,angle radius=0.5cm,angle eccentricity=2,red]
      {angle=f--c--d};
...
For a given set of model parameters, the likelihood of the data can be calculated using a recursive algorithm (the forward algorithm, cf.\ \citealp{pat15}), which in a very effective way considers all possible state sequences that might have given rise to the observed time series. This makes numerical maximization of the (log-)likelihood, and hence maximum likelihood estimation, feasible in most cases. Having estimated the model parameters (and examined useful diagnostics of model fit), the user can estimate the most likely sequence of behavioural states. We encourage the user to consult a good primary text such as \cite{zucchini2009} to familiarize themselves with the technical details of hidden Markov models. 
...
The elk data considered in \cite{morales2004} are available online at:
\begin{center}
...
The dataframe \texttt{trackData} now has four columns: ``Individual'', ``Easting'', ``Northing'', and ``dist\_water..meters.''. The last one is the distance of the animal to water, which for illustration purposes we want to include in the model as a covariate.
...
Besides, the easting and northing values are expressed in meters in the data, and we decide that we want to deal with distances in kilometers for the step lengths. To achieve this, we transform the coordinates into meters.
<<trackData4,size='small'>>=
trackData$Easting <- trackData$Easting/1000
trackData$Northing <- trackData$Northing/1000
@

\noindent As a result, this is what the data looks like:
<<trackData5,size='small'>>=
head(trackData)
@

\subsubsection{Use \texttt{prepData}}
With the data in the proper format, it can be processed using \texttt{prepData} to compute step lengths and angles. We choose the arguments carefully:
\begin{itemize}
...
We also need to specify initial values for the parameters of the state-dependent distributions, to be used by the optimization function. Note that this choice is crucial, and that \textbf{the algorithm might not find the global optimum of the likelihood function if the initial parameters are poorly chosen}. The initial parameters should be specified in two vectors, \texttt{stepPar0} (for the step distribution) and \texttt{anglePar0} (for the angle distribution). The necessary parameters of each distribution are detailed in Section \ref{sec_distrib}.
...
To choose initial values for the parameters, in the elk example, we consider that we want to identify two states corresponding to two levels of activity. In the first state, the animal moves slowly (i.e. the step lengths are short) and tends to stay in an area (i.e. the turning angles are centred around $\pi$). In the second state, the animal moves faster (i.e. the step lengths are longer) and tends to have a more directed movement (i.e. the turning angles are centred around $0$). Thus, in state 1, we choose a gamma(0.1,0.1) distribution with 5\% zero-inflation for the initial step distribution, and a vonMises($\pi$,1) distribution as the initial angle distribution. In state 2, we choose a gamma(1,1) distribution without zero-inflation for the steps, and a vonMises(0,1) distribution for the angles.

To declare the initial parameters, and fit the model to the elk data, we use:
<<fitHMM,size='small',eval=FALSE>>=
# initial parameters for gamma and von Mises distributions
mu0 <- c(0.1,1) # step mean (two parameters: one for each state)
sigma0 <- c(0.1,1) # step SD
zeromass0 <- c(0.05,0) # step zero-mass
...
\subsubsection{Dealing with numerical instability}
...
In terms of AIC, the 3-state model seems to largely outperform the 2-state model in this example.

\subsubsection{Model checking}
The pseudo-residuals (a.k.a.\ quantile residuals) of the model can be computed with \texttt{pseudoRes}. These follow a standard normal distribution if the fitted model is the true data-generating process. In other words, a deviation from normality indicates a lack of fit. For more theoretical background on pseudo-residuals, see \cite{zucchini2009}. The pseudo-residuals of the 2-state model fitted to the elk data are displayed in Figure \ref{fig:plotPR}. They can be computed and plotted with the following commands.
...
For the gamma distribution, the link between the mean/standard deviation (expected by \texttt{fitHMM}) and shape/scale (expected by \texttt{dgamma}) is given by:
\begin{align*}
  shape & = \dfrac{mean^2}{sd^2}\\
  scale & = \dfrac{sd^2}{mean}
\end{align*}
...
Tracking data usually consists of time series of either easting-northing coordinates or longitude-latitude values. However, with the HMM approach the derived quantities step lengths and turning angles are modelled. 
...
\item \texttt{pseudoRes} computes the pseudo-residuals of the model. These can be used to assess the goodness of fit. If the model is the true data-generating process, then the pseudo-residuals follow a standard normal distribution \citep{zucchini2009}.
...
\item \texttt{viterbi} computes the sequence of most probable states, under the fitted model, using the Viterbi algorithm \citep{zucchini2009}.
...
It is possible to call \texttt{simData} on a fitted model directly, to simulate data from it. This can be used to assess the fit, by checking that the simulated data displays the same features as the real data.


\newpage
\begin{thebibliography}{}

\bibitem[\protect\citeauthoryear{Jonsen et al.}{2005}]{jonsen2005}
  \textsc{Jonsen, I.D., Flemming, J.M., Myers, R.A.} (2005),
  ``Robust state-space modeling of animal movement data'',
  \textit{Ecology}, 86 (11), 2874--2880.
...
\bibitem[\protect\citeauthoryear{McClintock et al.}{2012}]{mcclintock2012}
  \textsc{McClintock, B.T., King, R., Thomas, L., Matthiopoulos, J., McConnell, B.J., Morales, J.M.} (2012),
  ``A general discrete-time modeling framework for animal movement using multistate random walks'',
  \textit{Ecological Monographs}, 82 (3), 335--349.
...
\bibitem[\protect\citeauthoryear{Patterson et al.}{2015}]{pat15}
  \textsc{Patterson T.A., Langrock, R., Blackwell, P.G., Thomas, L., Parton, A.} (2015),
