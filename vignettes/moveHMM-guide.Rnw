...
\begin{figure}[ht]
  ...
  \caption{Map of the animals' tracks (left) -- each color represents an animal. Graphs of the step lengths and turning angles for one individual, ``elk-115'' (right).}
  \label{moveData}
\end{figure}
...
Zero-inflation must be included in the step length distribution if some steps are of length exactly zero (which is the case for the elk data). To do so, another parameter is added to the step distribution: its mass on zero.
...
If some steps are exactly equal to zero, then strictly positive distributions such as the gamma are inadequate. In such cases, zero-inflated distributions can be considered. %With $l_t$ denoting the random variable giving the step length of an individual at time $t$, 
