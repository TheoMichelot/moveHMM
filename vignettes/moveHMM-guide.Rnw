...
\author{Michelot T., Langrock R., and Patterson T.}
...
For full details of the HMM approach in the context of animal movement modelling, the user should refer to the relevant primary publications (see, e.g., \citealp{patterson2009}, \citealp{langrock2012}, \citealp{zucchini16}). Here, we briefly highlight the essential features of the HMM approach. 
...
For a given set of model parameters, the likelihood of the data can be calculated using a recursive algorithm (the forward algorithm, cf.\ \citealp{patterson16}), which in a very effective way considers all possible state sequences that might have given rise to the observed time series. This makes numerical maximization of the (log-)likelihood, and hence maximum likelihood estimation, feasible in most cases. Having estimated the model parameters (and examined useful diagnostics of model fit), the user can estimate the most likely sequence of behavioural states. We encourage the user to consult a good primary text such as \cite{zucchini16} to familiarize themselves with the technical details of hidden Markov models. 
...
\begin{center}
  \url{http://www.esapubs.org/archive/ecol/E085/072/elk\_data.txt}
\end{center}
...
The data is in the proper format, and can be processed using \texttt{prepData} to compute step lengths and angles. We choose the arguments carefully:
\begin{itemize}
...
The pseudo-residuals (a.k.a.\ quantile residuals) of the model can be computed with \texttt{pseudoRes}. These follow a standard normal distribution if the fitted model is the true data-generating process. In other words, a deviation from normality indicates a lack of fit. For more theoretical background on pseudo-residuals, see \cite{zucchini16}. The pseudo-residuals of the 2-state model fitted to the elk data are displayed in Figure \ref{fig:plotPR}. They can be computed and plotted with the following commands.
...
The function \texttt{plotSat} plots the tracking data on a satellite image, with the help of the package ggmap \citep{kahle13}. Il only works with longitude and latitude values, so we need to convert the UTM coordinates first. We can do this e.g.\ with the help of the packages sp \citep{pebesma05} and rgdal \citep{bivand16} (more information can be found in the documentations of those packages).
<<plotSat1, size='small', message=FALSE>>=
library(rgdal)
...
In the code above, we need to multiply the UTM coordinates by 1000, as we had divided them by 1000 earlier to work with distances in kilometres. In the function \texttt{SpatialPoints}, we indicate \texttt{+zone=18}, because the data come from the UTM zone 18.

The data frame \texttt{lldata} contains the converted longitude and latitude coordinates of the observations. We can plot it with \texttt{plotSat}; the result is shown in Figure \ref{fig:plotSat2}. As the satellite image is fetched from Google, an Internet connection is required to use \texttt{plotSat}.
<<plotSat2, size='small', fig.width='4in', fig.height='4in', out.width='4in', out.height='4in', fig.pos='ht', fig.align='center', fig.cap="Plot of the elk data obtained with plotSat.", message=FALSE, cache=TRUE>>=
...
\item \texttt{pseudoRes} computes the pseudo-residuals of the model. These can be used to assess the goodness of fit. If the model is the true data-generating process, then the pseudo-residuals follow a standard normal distribution \citep{zucchini16}.
...
\item \texttt{viterbi} computes the sequence of most probable states, under the fitted model, using the Viterbi algorithm \citep{zucchini16}.
...
\bibitem[\protect\citeauthoryear{Kahle and Wickham}{2013}]{kahle13}
\textsc{Kahle, D., Wickham, H.} (2013) 
``ggmap: Spatial Visualization with ggplot2'',
\textit{The R Journal}, 5 (1), 144--161. 
...
\bibitem[\protect\citeauthoryear{Patterson et al.}{2016}]{patterson16}
  \textsc{Patterson T.A., Parton, A., Langrock, R., Blackwell, P.G., Thomas, L., King, R.} (2016),
  ``Statistical modelling of animal movement: a myopic review and a discussion of good practice'',
  \textit{arXiv preprint}, arXiv:1603.07511.

\bibitem[\protect\citeauthoryear{Pebesma and Bivand}{2005}]{pebesma05}
\textsc{Pebesma, E.J., Bivand, R.S.} (2005) 
``Classes and methods for spatial data in R'',
\textit{R News}, 5 (2), 144--161. 
URL: \url{http://cran.r-project.org/doc/Rnews/}

\bibitem[\protect\citeauthoryear{Zucchini et al.}{2016}]{zucchini16}
  \textsc{Zucchini, W.\, MacDonald, I.L., Langrock, R.} (2016).
  {\em Hidden Markov Models for Time Series: An Introduction Using R, Second Edition.}
